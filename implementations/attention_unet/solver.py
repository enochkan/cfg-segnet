...
class Solver(object):
	...
	def dice_loss_test(self, pred, target, smooth=1.):
	...
	def test(self, model_path, ds_name, fold):
		unet_path = model_path
		statistics = {}

		self.build_model()
		self.unet.load_state_dict(torch.load(unet_path))
		print('model_successfully loaded..')
		...
		for image, gt, aged in tqdm(self.test_loader):

			images = image.to(self.device)
			GT = gt.to(self.device)

			# SR : Segmentation Result
			mask = F.sigmoid(self.unet(images))
			...
			for ic in range(2):
				if 1. in torch.unique(GT[ic, :, :, :]):
					# print(torch.unsqueeze(mask[idx, :,:,:], 0).shape)
					dsc = float(1 - self.dice_loss_test(torch.unsqueeze(mask[ic, :, :, :], 0),
														  torch.unsqueeze(GT[ic, :, :, :], 0)).data.cpu().numpy())

					age = int(aged[ic].cpu().detach().int())
					age = str(age)
			...
			for id in range(mask.shape[0]):
				dh += directed_hausdorff(np.squeeze(mask.detach().cpu().numpy(), axis=1)[id, :, :],
										 np.squeeze(GT.cpu().numpy(), axis=1)[id, :, :])[0]
			if dh / mask.shape[0] != 0.0:
				hausdorffs.append(dh / mask.shape[0])

			idx += 1
			dice_score = float(1 - self.dice_loss_test(mask, GT).data.cpu().numpy())
			iou = (iou_pytorch(mask, GT).data.cpu().numpy())
			# print(iou)

			ious.append(iou)
			dice_scores.append(dice_score)
			img_sample = torch.cat((images.data, GT.data, mask.data), -2)
	...
	def train(self, fold):
		...
		if os.path.isfile(unet_path):
			...
			for epoch in range(50):
				unet_path = self.model_path + '/' + fold + '/' + str(epoch) +'.pkl'
