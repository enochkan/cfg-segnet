...
parser.add_argument("--dataset_name", type=str, default="pancreas_unconditioned", help="name of the dataset")
...
parser.add_argument("--lambda_A", type=float, default=10, help="generator A loss parameter")
parser.add_argument("--lambda_B", type=float, default=10, help="generator B loss parameter")
...
def sample_images(batches_done):
    ...
    # # # Threshold
    # fake_B.data[fake_B.data <= 0.65] = 0
    # fake_B.data[fake_B.data > 0.65] = 1

    AB = torch.cat((real_A.data, fake_B.data, recov_A), -2)
    BA = torch.cat((real_B.data, fake_A.data, recov_B), -2)
...
for epoch in range(opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        # Adversarial loss
        D_A_loss = -torch.mean(D_A(imgs_A)) + torch.mean(D_A(fake_A)) + lambda_A * gp_A
        ...
        # Adversarial loss
        D_B_loss = -torch.mean(D_B(imgs_B)) + torch.mean(D_B(fake_B)) + lambda_B * gp_B
