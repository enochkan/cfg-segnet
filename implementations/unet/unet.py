...
def run():
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        for idx, batch in enumerate(dataloader):
            ...
            dl = criterion_bce(mask_real, real_A)
            ...
            # Print log
            print(
                "\r[Epoch %d/%d] [CV %s] [Batch %d/%d] [Dice loss: %f] ETA: %s"
                % (
                    epoch,
                    opt.n_epochs,
                    opt.fold,
...
def test(model_epoch=None, model_dice=None, im_dim=256, ds_name='test', fold='fold2', n_classes=6):
    ...
    # generator.load_state_dict(torch.load('saved_models/'+ds_name+'/' + fold + '/' +'generator_'+str(model_epoch)+'_dice_'+str(model_dice)+'.pth'))
    # generator.eval()
    segmentor = UNet(n_channels=1, n_classes=1).cuda()
    segmentor.load_state_dict(torch.load('saved_models/prostate/'+ fold + '/' + 'segmentor_'+str(model_epoch)+'_dice_'+str(model_dice)+'.pth'))
    segmentor.eval()
    # discriminator = Discriminator(n_classes).cuda()
    # discriminator.load_state_dict(torch.load('saved_models/'+ds_name+'/'+fold + '/' + 'discriminator_'+str(model_epoch)+'_dice_'+str(model_dice)+'.pth'))
    # discriminator.eval()
...
if __name__ == '__main__':
    ...
    # run()
    # fold 1
    # test(model_epoch=41, model_dice=0.6501692611724138, ds_name='test', fold='fold1')
    # fold 2
    # test(model_epoch=24, model_dice=0.7822803131537512, ds_name='test', fold='fold2')

    # fold 3
    # test(model_epoch=20, model_dice=0.7805366388522088, ds_name='test', fold='fold3')
    # test(model_epoch=43, model_dice=0.8012820447795093, ds_name='test', fold='fold3')
    # fold 4
    # test(model_epoch=45, model_dice=0.6858553218655288, ds_name='test', fold='fold4')

    ## prostate
    # test(model_epoch=41, model_dice=0.957526603496323, ds_name='prostate_atlas', fold='fold1')
    # test(model_epoch=45, model_dice=0.8154984127730132, ds_name='prostate_atlas', fold='fold2')
    # test(model_epoch=44, model_dice=0.9648775426737431, ds_name='prostate_atlas', fold='fold3')
    test(model_epoch=32, model_dice=0.9372911107415954, ds_name='prostate_atlas', fold='fold4')
